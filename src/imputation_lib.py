...
def pseudo_gibbs(model, x, missing_mask, conditioner, condition_set, num_samples=10, num_iter=100, device="cpu"):
    ...
    for itx in range(num_iter):
        ...
        z_samples.append(z_rec["samples"][0,0].cpu().tolist())
        conditions_list.append(conditions[0].cpu().tolist())
...
def metropolis_within_gibbs(model, x, missing_mask, conditioner, condition_set, z_init=None, condition_init=None, num_samples=10, num_iter=100, verbose_freq=1000, device="cpu"):
    ...
    if z_init is None: z = model.encoder.sample(model.encoder(torch.cat((x, conditions), dim=1)))[0]
    else: z = z_init
    ...
    for itx in range(num_iter):
        ...
        if eps<rho:
            z = z_proposal*1.0
            x_params = x_params_proposal.copy()
            num_accepted_in_epoch += 1

        cond_sample, _, _ = sample_conditional(model.decoder, x_params, x[:,observed_indices], observed_indices)
            
        x[missing_mask] = cond_sample*1.0

        x_samples.append(x[0].cpu().tolist())
        z_samples.append(z[0].cpu().tolist())
        conditions_list.append(conditions[0].cpu().tolist())
        ...
        if (itx+1)%verbose_freq==0: print(f"Acceptance ratio: %{num_accepted_in_epoch/verbose_freq*100}")
    return np.array(x_samples), np.array(z_samples), np.array(conditions_list)
...
def cvae_imputation(model, data, conditioner, condition_set, num_samples=10, num_iter=100, warmup_steps=10, verbose_freq=1000, device="cpu"):
    ...
    if num_iter > 0:
        ...
        x_samples, z_samples, condition_samples = metropolis_within_gibbs(model, torch.tensor(x_samples[0])[None,...].float().to(device), 
                                                                          missing_mask, 
                                                                          conditioner, 
                                                                          condition_set, 
                                                                          z_init=torch.tensor(z_samples[0])[None,...].float().to(device), 
                                                                          condition_init=torch.tensor(condition_samples[0])[None,...].float().to(device), 
